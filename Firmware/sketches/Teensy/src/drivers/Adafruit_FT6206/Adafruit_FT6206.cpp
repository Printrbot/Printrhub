...
*/
/**************************************************************************/
boolean Adafruit_FT6206::begin(uint8_t interruptPin, uint8_t threshhold)
{
    _interruptPin = interruptPin;
    pinMode(interruptPin, INPUT);
    _lastPoint = TS_Point(0,0,0);

    Wire.begin();

    // change threshhold to be higher/lower
    writeRegister8(FT6206_REG_THRESHHOLD, threshhold);
    writeRegister8(FT6206_REG_G_MODE, FT6206_INTERRUPT_POLLING_MODE);
...
{
    bool touched = digitalRead(_interruptPin);
...
bool Adafruit_FT6206::readData(uint16_t *x, uint16_t *y)
{
